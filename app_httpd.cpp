...
volatile extern float gpsLat;
volatile extern float gpsLng;
volatile extern float gpsAlt;

volatile extern float compassX;
volatile extern float compassY;
volatile extern float compassZ;
volatile extern float compassHeading;
...
// Look at values within URL to determine function
  ...
  else if (!strcmp(variable, "turningAngle"))
  {
    if      (val > 180) val = 180;
    else if (val <   0) val = 0;
    turningAngle = val;
...
static esp_err_t status_handler(httpd_req_t *req) {
  ...
  *p++ = '{';
  ...
  p += sprintf(p, "\"turningAngle\":%d,", turningAngle);
...
static esp_err_t peripherals_handler(httpd_req_t *req) {
  static char json_response[1024];

  char * p = json_response;
  *p++ = '{';

  char gpsLatString[10];
  char gpsLngString[10];
  char gpsAltString[10];

  char compassXString[10];
  char compassYString[10];
  char compassZString[10];
  char compassHeadingString[10];

/* 7 is mininum width, 4 is precision; float value is copied onto str_temp*/
dtostrf(gpsLat, 7, 4, gpsLatString);
dtostrf(gpsLng, 7, 4, gpsLngString);
dtostrf(gpsAlt, 7, 4, gpsAltString);

dtostrf(compassX, 7, 4, compassXString);
dtostrf(compassY, 7, 4, compassYString);
dtostrf(compassZ, 7, 4, compassZString);
dtostrf(compassHeading, 7, 4, compassHeadingString);

  p += sprintf(p, "\"gpsLat\":%s,", gpsLatString);
  p += sprintf(p, "\"gpsLng\":%s,", gpsLngString);
  p += sprintf(p, "\"gpsAlt\":%s,", gpsAltString);
  p += sprintf(p, "\"compassX\":%s,", compassXString);
  p += sprintf(p, "\"compassY\":%s,", compassYString);
  p += sprintf(p, "\"compassZ\":%s,", compassZString);
  p += sprintf(p, "\"compassHeading\":%s", compassHeadingString);
...
<html>
    ...
    <br>
            <h1>GPS</h1>
            <p id="gpsLat">Latitude: </p>
            <p id="gpsLng">Longitude: </p>
            <p id="gpsAlt">Altitude: </p>

            <h1>Compass</h1>
            <p id="compassX">Compass X: </p>
        ...
        fetch("http://192.168.4.1/peripherals")
          .then(response => response.json())
          .then(data => {
            
            document.getElementById("gpsLat").innerHTML = String("Latitude: " + data.gpsLat);
            document.getElementById("gpsLng").innerHTML = String("Longitude: " + data.gpsLng);
            document.getElementById("gpsAlt").innerHTML = String("Altitude: " + data.gpsAlt + " meters");
            document.getElementById("compassX").innerHTML = String("Compass X: " + data.compassX);
            document.getElementById("compassY").innerHTML = String("Compass Y: " + data.compassY);
...
void startCameraServer()
{
   ...
   httpd_uri_t stream_uri = {
    ...
    httpd_uri_t peripherals_uri = {
        .uri       = "/peripherals",
        .method    = HTTP_GET,
        .handler   = peripherals_handler,
        .user_ctx  = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &peripherals_uri);
    }
